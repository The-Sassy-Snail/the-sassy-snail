...
// (in their own timezone) is close to their configured morning/evening
// reminder time and hasn't already fired today, sends a push notification
// via Firebase Cloud Messaging to every device they've enabled notifications
// on. This script has full admin access to Firestore (via a service account
// secret) and bypasses the app's normal per-user security rules — it never
// runs in the browser.
...
async function main() {
  ...
  for (const docSnap of notifyDocs) {
    ...
    if (!data.enabled || tokens.length === 0) continue;
    ...
    for (const slot of SLOTS) {
      ...
      const diff = Math.abs(nowMin - toMinutes(target));
      if (diff > WINDOW_MINUTES) continue;

      console.log(`Sending ${slot.key} reminder to uid=${uid} (${tokens.length} device(s))`);
      ...
      const update = { [slot.sentField]: dateStr };
